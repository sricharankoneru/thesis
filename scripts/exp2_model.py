...
class DeepLab(pl.LightningModule):
    def __init__(self, arch, encoder_name, in_channels, out_classes, **kwargs):
        ...
        # Change loss function to Binary Cross Entropy with logits
        #self.loss_fn = torch.nn.BCEWithLogitsLoss()
        self.loss_fn = DiceLoss(sigmoid=True)
        self.logged_image = False
        self.random_batch_idx = None
        self.output_dir = os.path.join(os.getcwd(), 'jupyter/predictions/xception-71-dl/Predict')
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred_binary = (pred_prob > 0.45).int()
        y_binary = y.int()
